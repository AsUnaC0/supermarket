...
<script>
import { getAddressList, getDefalutAddressid } from '@/api/address.js'
...
export default {
  ...
  computed: {
    selectedAddress () {
      let address
      Object.values(this.addressList).forEach(value => {
        if (value.address_id === this.addressid) {
          address = value
        }
      })
      return address || {}
  ...
  methods: {
    ...
    async getDefalutAddress () {
      const res = await getDefalutAddressid()
      this.addressid = res.data.defaultId
    },
    async getOrderList () {
      // 购物车结算
      if (this.mode === 'cart') {
